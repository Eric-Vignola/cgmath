...
@njit(parallel=True, fastmath=True, cache=True)
def _remap_array_1d_parallel(
    arr:   np.ndarray,
    t_min: float,
    t_max: float,
) -> np.ndarray:
    """
    Remap 1D array values to [t_min, t_max] range in parallel.

    Parameters
    ----------
    arr : np.ndarray
        Input 1D array, float64.
    t_min : float
    ...
    scale = (t_max - t_min) / (arr_max - arr_min)
    for i in prange(n):
        result[i] = t_min + (arr[i] - arr_min) * scale

    return result


@njit(parallel=True, fastmath=True, cache=True)
def _remap_array_2d_parallel(
    arr:   np.ndarray,
    t_min: np.ndarray,
    t_max: np.ndarray,
) -> np.ndarray:
    """
    Remap 2D array values along last axis in parallel.

    Each row is independently remapped to its corresponding [t_min, t_max] range.

    Parameters
    ----------
    arr : np.ndarray
        Input 2D array of shape (N, M), float64.
    t_min : np.ndarray
        Target minimum values, shape (N,), float64.
    t_max : np.ndarray
        Target maximum values, shape (N,), float64.

    Returns
    -------
    np.ndarray
        Remapped array of shape (N, M), float64.
    """
    n_rows = arr.shape[0]
    n_cols = arr.shape[1]
    ...
    for i in prange(n_rows):
        ...
        if row_max == row_min:
            mid = (t_min[i] + t_max[i]) / 2.0
            for j in range(n_cols):
                result[i, j] = mid
        else:
            scale = (t_max[i] - t_min[i]) / (row_max - row_min)
            for j in range(n_cols):
                result[i, j] = t_min[i] + (arr[i, j] - row_min) * scale

    return result


def remap_array_fast(
    arr:   np.ndarray,
    t_min: float      | np.ndarray = 0.0,
    t_max: float      | np.ndarray = 1.0,
) -> np.ndarray:
    """
    Fast version of remap_array using parallel Numba kernels.
    ...
    t_min : float or np.ndarray
        Target minimum value(s).
    t_max : float or np.ndarray
        Target maximum value(s).
    ...
    if arr.ndim == 1:
        t_min_val = float(t_min) if np.isscalar(t_min) else float(t_min.flat[0])
        t_max_val = float(t_max) if np.isscalar(t_max) else float(t_max.flat[0])
        return _remap_array_1d_parallel(arr, t_min_val, t_max_val)

    elif arr.ndim == 2:
        n_rows = arr.shape[0]

        if np.isscalar(t_min):
            t_min_arr = np.full(n_rows, float(t_min), dtype=np.float64)
        else:
            t_min_arr = np.asarray(t_min, dtype=np.float64).ravel()
            if len(t_min_arr) == 1:
                t_min_arr = np.full(n_rows, t_min_arr[0], dtype=np.float64)
            elif len(t_min_arr) != n_rows:
                raise ValueError(
                    f"t_min length {len(t_min_arr)} does not match number of rows {n_rows}"
                )

        if np.isscalar(t_max):
            t_max_arr = np.full(n_rows, float(t_max), dtype=np.float64)
        else:
            t_max_arr = np.asarray(t_max, dtype=np.float64).ravel()
            if len(t_max_arr) == 1:
                t_max_arr = np.full(n_rows, t_max_arr[0], dtype=np.float64)
            elif len(t_max_arr) != n_rows:
                raise ValueError(
                    f"t_max length {len(t_max_arr)} does not match number of rows {n_rows}"
                )

        return _remap_array_2d_parallel(arr, t_min_arr, t_max_arr)

    else:
        raise ValueError(
            f"remap_array_fast only supports 1D and 2D arrays, got {arr.ndim}D"
        )
